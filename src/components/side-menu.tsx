...
import { Plus, BookOpen, Menu, X, User, Hammer } from 'lucide-react'; // Added User and Hammer back
  ...
  return (
    <Link href={href} passHref>
      <motion.div
        ...
        `}
        whileHover={{ scale: 1.05, boxShadow: isActive ? '0 10px 15px -3px rgba(var(--highlight-rgb), 0.5), 0 4px 6px -2px rgba(var(--highlight-dark-rgb), 0.5)' : '0 10px 15px -3px rgba(var(--primary-rgb), 0.3), 0 4px 6px -2px rgba(var(--primary-rgb), 0.2)' }}
        whileTap={{ scale: 0.95 }}
        title={label}
      >
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center gap-6 p-4 flex-grow">
          ...
          {/* Added Mi Perfil and Ajustes to mobile menu */}
          ...
          {user && (
            ...
            >
              ...
              <p className="text-sm italic mb-4">Total: <span className="font-bold text-[var(--highlight)] text-xl">{totalTokens}</span></p>
      ...
      <aside
        ...
        `}
      >
        {/* Added a div to control spacing for the main navigation items */}
        <div className="flex flex-col gap-6 w-full px-2 mt-4"> {/* Changed space-y-12 to gap-6 and added flex-col */}
          <SideMenuItem href="/kitchen" icon={<Plus />} label="Nueva" />
          <SideMenuItem href="/kitchen/recipes/list" icon={<BookOpen />} label="Mis Recetas" />
        </div>
      </aside>
    </>
  );
